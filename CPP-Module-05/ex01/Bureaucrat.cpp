...
{
	return "[BUREAUCRAT Exception] Grade can't be that high!";
}

const char	*Bureaucrat::GradeTooLowException::what() const throw()
{
	return "[BUREAUCRAT Exception] Grade can't be that low!";
}
...
void Bureaucrat::signForm(Form &var)
{
	...
	{
		std::cout << this->getName() << " couldn't sign " << var.getName() << " because " << e.what() << std::endl;
		return;
	}
	std::cout << this->getName() << " has signed " << var.getName() << std::endl;
}